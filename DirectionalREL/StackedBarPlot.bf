							  ...
							  )	
{
	...
	numberOfComponents = Columns (xy)-1;
	_x 				   = Rows (xy);
	...
	for (_dataPoint = 0; _dataPoint < numberOfComponents; _dataPoint = _dataPoint + 1)
	{
	...
	for (_dataPoint = 0; _dataPoint < _x; _dataPoint = _dataPoint + 1)
	{
		...
		for (_segment = 0; _segment < numberOfComponents; _segment = _segment + 1)
		{
			blah = xy[_dataPoint][_segment];
		
			if (blah>0)
			{
				_seg_height = xy[_dataPoint][_segment] * py / xy[_dataPoint][numberOfComponents];
		...
		psDensityPlot * ("" + (plotOriginX+(_dataPoint+0.5)*px) + " " + (plotOriginY + barHeight + 0.5*plotDim[2]) +" (" + xy[_dataPoint][numberOfComponents] + ") centertext\n");
